...
- **SEO** — `title` и `meta description` для каждой публичной страницы (главная, каталог, отзыв, правила и т.д.). Для страницы отзыва (`/reviews/[id]`) в шаблонах можно писать плейсхолдеры `{title}`, `{city}`, `{excerpt}` — они подставляются из данных конкретного отзыва. Подробная инструкция с примерами — в `/admin/settings` (блок «Как работают плейсхолдеры»).
